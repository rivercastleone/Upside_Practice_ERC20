...
- **0x19 prefix + domain_separator + 구조화된 데이터를 해시한 형태**
    - 이더리움 트랜잭션은 일반적으로 `RLP 인코딩된 트랜잭션 데이터`와 이에 대한 `서명(v, r, s)`으로 구성
    - 오프체인에서 서명된 데이터가 유효한 `RLP 데이터`로 변환되고, 올바른 `서명(v, r, s)`과 함께 제출되면 유효한 트랜잭션으로 인식될 수 있음
    - 따라서 RLP encoding이 아님을 알리기 위해 Prefix 값 `0x19`를 붙여서 해시화 하는데 뒤의 값 `0x01`은 Structured data를 사용하고 싶을 경우에 사용함
